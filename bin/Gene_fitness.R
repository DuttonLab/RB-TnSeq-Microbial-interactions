##### INITIAL GENE FITNESS CALCULATION #####

#This function first calculates strain fitness and then raw gene fitness values

Gene_fitness=function(Data,cdnbF,genes.tab,org_locId){
...
#Then we substract the corresponding T0 values to each condition - Before substracting we want to make surecthat both tables are sorted the same way, so we order the dat based on the pos column 
  ...
  if(cdnbF==1){
    colnames(Table_GeneFitness_MEAN)=c("locusId",colnames(StrainFit_Table[8]))
  }
  ...
  if(cdnbF==1){
    colnames(Table_GeneFitness_VAR)=c("locusId",colnames(StrainFit_Table[8]))
  }
