...
Correlation_Rep=function(AllReplicate){
  ...
  for(i in 1:length(Condition)){
    ...
    for(j in 1:dim(Combi_mat)[2]){
      ...
      CorP_val=cor(Set1$Fitness_byMean, Set2$Fitness_byMean, method=c("pearson"))
      CorS_val=cor(Set1$Fitness_byMean, Set2$Fitness_byMean, method=c("spearman"))
      CorL_val=CCC(Set1$Fitness_byMean, Set2$Fitness_byMean)
      
      Data_plot=left_join(Set1,Set2,by=c("locusId"))
      
      plotcor=ggplot(Data_plot, aes(x=Fitness_byMean.x, y=Fitness_byMean.y )) + geom_point(size=0.5, shape=20)+
        geom_abline(intercept = 0, slope = 1, col="red", linetype='dashed') + theme_light()+
        geom_text(x=0, y=min(Data_plot$Fitness_byMode.y), label=paste("R2 = ", CorP_val), col="red")+
