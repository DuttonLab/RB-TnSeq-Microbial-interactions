...
#Student comparison test can only be performed on samples with equal variance (squared standard deviation), we first have to identify which genes fitness have equal variance between the conditions
...
#We use the Fisher statistics for alphaF=5%  (so F0.025 since we do bilateral test) or alphaF=0.2% (so F0.001) for constant degrees of freedom of 2 and 2 since we have 3 replicates

Comparison_test=function(Data_Fitness, Condition1, Condition2, alphaT, alphaF, org_locId, multi){
  ...
  for (i in 1:length(Locus_list)){
    ...
    V1=max((DatC1$Weightedsdev)^2,(DatC2$Weightedsdev)^2)
    V2=min((DatC1$Weightedsdev)^2,(DatC2$Weightedsdev)^2)
  ...
  if (alphaF==0.002){
    Data_FcalcGOOD=subset(Data_Fcalc, Data_Fcalc$Fcalc<=999)
  }
  ...
  if(org_locId=="Num"){
    ...
    for (i in 1:length(Locus_list2)){
      ...
      Var1=(as.numeric(DatC1$Weightedsdev))^2
      Var2=(as.numeric(DatC2$Weightedsdev))^2
    
      Fit1=as.numeric(DatC1$WeightedFit)
      Fit2=as.numeric(DatC2$WeightedFit)
  ...
  colnames(Data_test)[8:15]=c("Fit_C1","Var_C1","SD_C1","Cond1","Fit_C2","Var_C2","SD_C2","Cond2")
  
  Data_test=left_join(Data_test, Data_Fcalc, by=c("locusId"))
  Data_test=left_join(Data_test, Data_Tcalc, by=c("locusId"))
  
  # Pvalue correspondance, and correction for multiple comparison testing ==> adjusted pvalue
  
  Data_test$pvalue=2*pt(-abs(Data_test$Tcalc),4)
  Data_test$adjpvalue_fdr=p.adjust(Data_test$pvalue,method="fdr")
  
  # Category definition and plot
  alpha=alphaT
  Data_test=Category_definition(Data_test, alpha, multi)  #here you can change the value of alpha you want to use (0.05, 0.01, 0.005, 0.001)
